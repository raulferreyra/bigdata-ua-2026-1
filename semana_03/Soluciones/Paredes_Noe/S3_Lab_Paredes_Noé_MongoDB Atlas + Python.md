...
RESPUESTA A PREGUNTA DE REFLEXIÓN 1.1
============================================================
1. El documento de la empresa de tecnología tiene 14 campos, mientras que la empresa agroindustrial tiene 13 campos.
2. En una base de datos SQL tradicional (relacional), si la empresa agroindustrial no tuviera el campo contacto.web, esa columna tendría que rellenarse obligatoriamente con un valor 'NULL'.
Además, la tabla SQL tendría columnas vacías (NULL) para todos los campos agrícolas en la fila de tecnología, desperdiciando espacio.
En cambio, en MongoDB NoSQL, el campo simplemente NO existe en el documento, lo que ahorra almacenamiento y da flexibilidad
---
...
Respuesta:
insert_many() es drásticamente más eficiente por el Round Trip Time (RTT) o viaje de ida y vuelta de la red.

Si usáramos insert_one() en un bucle de 100 iteraciones, Google Colab tendría que hacer 100 viajes individuales por internet hasta el servidor de MongoDB Atlas en São Paulo (enviar 1 documento, esperar confirmación, enviar otro...). Esto multiplica el "overhead" (retraso) de red por 100, haciendo el proceso muy lento.
Con insert_many(), el driver empaqueta los 100 documentos en un único viaje de red (un solo request de red). El servidor los procesa juntos y devuelve una sola respuesta. Menos viajes por internet significan un rendimiento óptimo en arquitecturas Big Data Cloud
...
> Con 100 documentos la diferencia de velocidad con/sin índice es pequeña. ¿Por qué con 1 millón de documentos la diferencia sería dramática? ¿Qué estructura de datos usa MongoDB internamente para sus índices (similar a HBase Row Key)?

Respuesta:¿Por qué con 1 millón de documentos la diferencia sería dramática?
Con solo 100 documentos, la computadora realiza una operación conocida como COLLSCAN (Collection Scan o Escaneo de Colección completa). Como 100 registros caben por completo en la memoria caché del procesador, leerlos uno por uno toma apenas fracciones de milisegundo, haciendo que la diferencia con un índice parezca insignificante.Sin embargo, cuando la colección escala a 1 millón de documentos:Sin Índice (Complejidad Lineal $O(N)$): MongoDB se ve obligado a realizar una lectura secuencial en el disco por cada consulta. Debe evaluar un millón de registros uno a uno para verificar cuáles cumplen el criterio. Si realizas 100 consultas simultáneas, el sistema tendría que procesar 100 millones de evaluaciones, saturando los discos y disparando los tiempos de respuesta a segundos o minutos.Con Índice (Complejidad Logarítmica $O(\log N)$): El motor no lee el disco a ciegas, sino que salta directo a las ubicaciones exactas mediante un puntero de memoria. En lugar de un millón de operaciones, resolver la consulta toma alrededor de 15 a 20 operaciones de búsqueda, manteniendo la respuesta por debajo de los 2 milisegundos sin importar el tamaño del dataset.
2. ¿Qué estructura de datos usa MongoDB internamente para sus índices?MongoDB utiliza internamente una estructura de datos balanceada llamada Árbol B (B-Tree) (específicamente a través de su motor de almacenamiento por defecto, WiredTiger).Comparación con HBase: A diferencia de Apache HBase (que organiza sus datos en llaves jerárquicas llamadas Row Keys ordenadas mediante tablas LSM-Trees optimizadas para escrituras secuenciales masivas en HDFS), el B-Tree de MongoDB está diseñado para lecturas puntuales ultra rápidas y rangos dinámicos en cualquier campo del JSON.El árbol se auto-balancea de forma constante en memoria RAM. Cada nodo contiene llaves ordenadas y punteros a los documentos físicos en el disco, permitiendo que el motor descarte millones de opciones de búsqueda con solo recorrer un par de niveles del árbol jerárquico
...
MongoDB fue infinitamente más simple. Mientras que en SQL tuvimos que romper la lógica del negocio creando dos tablas separadas mediante llaves foráneas (FOREIGN KEY) y luego cruzarlas con un costo de rendimiento alto usando un JOIN, en MongoDB guardamos la estructura tal y como nace de forma nativa. La consulta se resolvió en una sola línea usando notación punto ($facturacion_anual.2024) sin recurrir a cruces de tablas.
...
> ¿Qué datos de tu proyecto grupal (Grupos 1-7) almacenarías en MongoDB en lugar de en una tabla SQL? ¿Qué campo sería el equivalente al "_id" o clave de acceso principal? ¿Tendría sentido usar el Aggregation Pipeline de MongoDB en lugar de PySpark para algún análisis específico de tu proyect
---
