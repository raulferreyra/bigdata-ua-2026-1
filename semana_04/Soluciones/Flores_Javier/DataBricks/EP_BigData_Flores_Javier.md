...
|---|---|---|---|
| **APELLIDOS Y NOMBRES:** |Flores Condeña Javier | **FECHA ENTREGA:** | 26/06/2026 |
...
|------------------------|-------------------|--------------------|--------------------------------------------|
| Core de pagos (3.2M transacciones/día, no puede perder dinero) | CockroachDB / PostgreSQL (Alta Disp.)| NewSQL / Relacional Distribuida|Soporta un montón de transacciones a la vez en hora punta y garantiza que ninguna transferencia se quede a medias o falle. |
| Sesiones de login activo (15M usuarios, expira en 30 min) |Redis |En memoria (In-Memory Key-Value) |Evita que la aplicación se ponga lenta cuando todos se conectan y cierra las cuentas inactivas por seguridad. |
| Perfil del comerciante (bodega, restaurante, taxi — atributos distintos) |MongoDB Atlas |NoSQL Orientada a Documentos |Nos da la flexibilidad de guardar los datos de cada negocio a su medida, sin tener una base de datos llena de celdas vacías. |
| Historial de transacciones para análisis (18 TB/año) |Databricks (Delta Lake) sobre AWS S3 |Data Lakehouse |Almacena masivamente datos crudos a bajo costo en formato Parquet enriquecido, optimizando consultas mediante la arquitectura Medallion. |
| Red de detección de fraude (ciclo A→B→C→A en < 5 min) |Neo4j / Amazon Neptune |NoSQL Orientada a Grafos |Ejecuta consultas de grafos multidimensionales en milisegundos de forma nativa. Evita la sobrecarga de procesamiento y los múltiples JOINs recursivos que degradarían un motor relacional. |
| Dashboard ejecutivo (top 10 distritos, actualización diaria) |Apache Spark SQL / Power BI |Motor Analítico + BI |Ejecuta pipelines de agregación masiva sobre tablas optimizadas en la capa Gold. Centraliza el procesamiento pesado en un único lote (batch) diario para evitar sobrecargar los sistemas transaccionales. |
...
|------------|----------------|----------------------|----------------------------------------------------------------|
| Core de pagos (débito/crédito de saldos) |CP (Consistencia - Partición) |Disponibilidad (A) |Es Correcto. Si hay una partición de red, es preferible rechazar la transacción temporalmente antes que generar un doble gasto o descuadres de dinero entre cuentas. |
| Historial "mis últimas 50 transacciones" |AP (Disponibilidad - Partición) |Consistencia Consistente (C) |Es Correcto. Es aceptable una consistencia eventual. Si el usuario no ve reflejado su último pago en el historial por unos segundos, el sistema sigue siendo usable y no altera los saldos reales. |
...
*Resuelve la limitación del escalamiento vertical clásico (hardware costoso y un único punto de falla centralizado). CockroachDB escala de manera horizontal y nativa, permitiendo distribuir la carga de transacciones concurrentes en múltiples nodos estándar geodistribuidos sin perder la estructura relacional.*

b) ¿Por qué MongoDB NO puede reemplazar a Oracle para el procesamiento de pagos aunque también escala horizontalmente?

*Porque el procesamiento de transacciones financieras complejas requiere de transacciones multi-documento con un aislamiento e integridad relacional estricta bajo estándares ACID globales rigurosos. Aunque MongoDB ha mejorado en soporte ACID, no está optimizado ni diseñado de raíz para la consistencia transaccional bancaria distribuida de alta concurrencia que exige un core de pagos.*
...
*Usa el protocolo de consenso Raft (junto con el control de concurrencia MVCC).*
