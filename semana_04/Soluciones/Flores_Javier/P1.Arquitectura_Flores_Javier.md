...
```
*El equipo de Yape usaría MongoDB en Docker para armar sus entornos locales de desarrollo y correr pruebas rápidas en sus máquinas o en pipelines de CI/CD antes de lanzar código. Es una jugada genial para experimentar, porque los devs pueden levantar y tumbar bases de datos idénticas en segundos en sus propias laptops sin gastar en la nube ni depender de que el internet esté al 100%, dejando MongoDB Atlas de forma exclusiva para la producción del negocio.*

b) ¿Qué ventaja tiene Atlas M0 sobre el contenedor Docker para el contexto universitario?

*Para nosotros en la universidad, Atlas M0 es un golazo porque te quita de encima el dolor de cabeza de habilitar la virtualización de hardware y no te consume memoria RAM ni espacio en el disco local. Al estar todo guardado de forma permanente y gratis en la nube, el grupo puede avanzar el proyecto desde el campus o desde casa usando la misma base de datos, conectándose en simultáneo sin enredarse con mapeo de puertos ni configuraciones de red.*
...
*Si le metes un docker stop y luego un docker rm, básicamente destruyes el contenedor y toda tu data local se borra para siempre porque su almacenamiento interno era efímero al no haberle configurado un volumen persistente. En cambio, con MongoDB Atlas no pasa absolutamente nada; tus documentos se quedan intactos, blindados y disponibles en la nube porque es un servicio administrado que corre de forma 100% independiente a los comandos que borres en tu terminal de Cloud Shell.*
