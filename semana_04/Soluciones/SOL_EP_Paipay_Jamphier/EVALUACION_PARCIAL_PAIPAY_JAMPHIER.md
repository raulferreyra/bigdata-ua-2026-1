...
| **CÓDIGO DEL ESTUDIANTE:** |2221894941 | **NÚMERO DE CLASE:** | |
|---|---|---|---|
| **APELLIDOS Y NOMBRES:** |JAMPHIER PAIPAY ANGELES | **FECHA ENTREGA:** | 26/06/2026 |
| **DOCENTE:** | **Mg. Rubén Quispe Llacctarimay** | **Modalidad:** | **Implementación + Video** |

---

ENLACE DE REPOSITORIO GIT : https://github.com/Jamphier-PA/YAPE-PAIPAY-JAMPHIER/tree/main
...
ENLACE DE ARQUITECTURA : https://drive.google.com/file/d/1ufPCGuXTPynZ7rIHp3ZpF0-8L148c4_w/view?usp=sharing

| Componente del sistema | Tecnología elegida | Tipo BD/Herramienta | Por qué esta tecnología para Yape (2 líneas) |
|------------------------|-------------------|--------------------|--------------------------------------------|
| Core de pagos (3.2M transacciones/día, no puede perder dinero) |CockroachDB |NewSQL |Elegí CockroachDB porque garantiza transacciones ACID, lo que asegura que cada pago se procese correctamente sin pérdida ni duplicidad de información. Además, permite escalar horizontalmente agregando nuevos nodos sin afectar la disponibilidad del servicio, característica importante para soportar el crecimiento de usuarios de Yape. |
| Sesiones de login activo (15M usuarios, expira en 30 min) |Redis |Base de datos en memoria |Redis almacena la información en memoria, por lo que ofrece tiempos de respuesta muy rápidos. Es ideal para manejar sesiones de usuarios, tokens de autenticación y datos temporales, permitiendo que la aplicación responda de forma inmediata incluso con millones de usuarios conectados. |
| Perfil del comerciante (bodega, restaurante, taxi — atributos distintos) |MongoDB |Base de datos documental (NoSQL) |MongoDB utiliza documentos con esquema flexible, permitiendo almacenar diferentes tipos de comercios sin modificar la estructura de la base de datos. Por ejemplo, una farmacia puede tener campos distintos a un restaurante o una bodega, lo que facilita la evolución de la aplicación. |
| Historial de transacciones para análisis (18 TB/año) |Databricks (Apache Spark + Delta Lake) |Big Data |Esta plataforma permite procesar grandes volúmenes de datos de manera distribuida y eficiente. Es adecuada para generar reportes, análisis históricos y modelos de inteligencia de negocios sin afectar el rendimiento del sistema transaccional. |
| Red de detección de fraude (ciclo A→B→C→A en < 5 min) |Neo4j |Base de datos de grafos |Neo4j representa las relaciones entre usuarios, dispositivos, cuentas y transacciones mediante grafos. Esto permite identificar rápidamente patrones sospechosos, conexiones entre cuentas y posibles fraudes que serían difíciles de detectar con una base de datos relacional. |
| Dashboard ejecutivo (top 10 distritos, actualización diaria) |Databricks SQL |Motor analítico |Databricks SQL permite consultar los datos procesados en la capa Gold y generar indicadores como número de transacciones, volumen de pagos, ticket promedio y comercios más activos. Facilita la creación de reportes y paneles para la toma de decisiones. |
...
|------------|----------------|----------------------|----------------------------------------------------------------|
| Core de pagos (débito/crédito de saldos) |CP |Disponibilidad |Si ocurre una partición de red, es preferible rechazar temporalmente una transacción antes que registrar pagos duplicados o inconsistentes. La consistencia es prioritaria en operaciones financieras. |
| Historial "mis últimas 50 transacciones" |AP |Consistencia inmediata |Es aceptable que algunas transacciones recientes tarden unos segundos en sincronizarse, siempre que el servicio permanezca disponible para los usuarios. |
...
Oracle escala principalmente mediante hardware más potente (escalamiento vertical), lo que incrementa significativamente los costos y presenta limitaciones al crecer la cantidad de usuarios. CockroachDB permite escalar horizontalmente agregando nuevos nodos al clúster sin detener el servicio, soportando un crecimiento de 15 a 50 millones de usuarios.

b) ¿Por qué MongoDB NO puede reemplazar a Oracle para el procesamiento de pagos aunque también escala horizontalmente?

MongoDB no es la mejor alternativa para reemplazar Oracle en el procesamiento de pagos porque, aunque soporta transacciones ACID, está diseñado principalmente para datos semiestructurados y aplicaciones con alta escalabilidad. Los sistemas financieros requieren consistencia estricta, aislamiento y control transaccional continuo, características donde una base de datos NewSQL como CockroachDB resulta más adecuada.
...
Raft es el algoritmo de consenso utilizado por CockroachDB para mantener la consistencia y replicación de los datos entre múltiples nodos distribuidos.
...
MongoDB en Docker se utiliza durante el desarrollo y las pruebas locales porque permite crear rápidamente un entorno idéntico al de producción sin depender de Internet. Además, facilita realizar pruebas, eliminar el contenedor y recrearlo cuando sea necesario, garantizando que todos los desarrolladores trabajen bajo la misma configuración.

b) ¿Qué ventaja tiene Atlas M0 sobre el contenedor Docker para el contexto universitario?

MongoDB Atlas M0 ofrece una base de datos administrada en la nube, con acceso desde cualquier lugar, copias de seguridad, monitoreo y alta disponibilidad sin necesidad de administrar servidores. Para un entorno universitario resulta ideal porque es gratuito y simplifica el despliegue y la colaboración entre estudiantes.
...
el contenedor únicamente se detiene y los datos permanecen almacenados.

Si posteriormente se ejecuta:

docker rm yape-mongo-local

sin utilizar un volumen persistente (docker volume), toda la información almacenada dentro del contenedor se elimina. En cambio, los datos almacenados en MongoDB Atlas permanecen disponibles porque se encuentran alojados en la nube y no dependen del contenedor local.
...
**Enlace del video:** `https://drive.google.com/file/d/1dK5r7-LuTc8r5kGSFJczXkGbi6ubME78/view?usp=sharing`
