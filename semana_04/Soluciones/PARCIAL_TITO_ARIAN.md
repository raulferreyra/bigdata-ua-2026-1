...
| **CÓDIGO DEL ESTUDIANTE:** |2221896270 | **NÚMERO DE CLASE:** | |
|---|---|---|---|
| **APELLIDOS Y NOMBRES:** |TITO PAREDES | **FECHA ENTREGA:** | 27//06/26|
| **DOCENTE:** | **Mg. Rubén Quispe Llacctarimay** | **Modalidad:** | **Implementación + Video** |


link de video:https://youtu.be/algUXriVvvg?si=C2E6oMuHlhQ9rC3s
link git:https://github.com/arian240504-ux/Examen-Parcial-Big-data-Arian-Tito
...
| ------------------------------------------------------------------------ | ----------------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Core de pagos (3.2M transacciones/día, no puede perder dinero)           | CockroachDB             | NewSQL                    | Garantiza transacciones ACID y alta consistencia. Permite escalar horizontalmente sin perder integridad de datos financieros. |
| Sesiones de login activo (15M usuarios, expira en 30 min)                | Redis                   | Key-Value Database        | Maneja datos temporales en memoria con acceso extremadamente rápido. Ideal para sesiones con expiración automática.           |
| Perfil del comerciante (bodega, restaurante, taxi — atributos distintos) | MongoDB                 | NoSQL Document Database   | Permite almacenar documentos flexibles con diferentes atributos según cada tipo de negocio sin esquema fijo.                  |
| Historial de transacciones para análisis (18 TB/año)                     | Databricks + Data Lake  | Big Data / Data Lakehouse | Procesa grandes volúmenes de datos históricos de manera distribuida. Ideal para análisis masivo y procesamiento batch.        |
| Red de detección de fraude (ciclo A→B→C→A en < 5 min)                    | Neo4j                   | Graph Database            | Permite analizar relaciones complejas entre usuarios y detectar patrones sospechosos o ciclos fraudulentos rápidamente.       |
| Dashboard ejecutivo (top 10 distritos, actualización diaria)             | Apache Spark + Power BI | Analytics / BI            | Spark procesa grandes conjuntos de datos diariamente y Power BI permite visualizar métricas y dashboards ejecutivos.          |
...
| ---------------------------------------- | --------------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Core de pagos (débito/crédito de saldos) | CP              | Availability (Disponibilidad) | En pagos financieros la prioridad es mantener consistencia absoluta. Es aceptable sacrificar disponibilidad temporal porque no se puede permitir pérdida o inconsistencia de dinero entre cuentas. |
| Historial "mis últimas 50 transacciones" | AP              | Consistency (Consistencia)    | Es aceptable mostrar transacciones con unos segundos de retraso. El usuario puede tolerar pequeña inconsistencia temporal mientras el sistema mantiene alta disponibilidad.                        |
...
Oracle normalmente depende de escalado vertical (más CPU o más memoria en un solo servidor), lo cual genera altos costos y límites físicos. CockroachDB permite escalado horizontal agregando múltiples nodos distribuidos para soportar millones de usuarios sin depender de un solo servidor.

---

### b) ¿Por qué MongoDB NO puede reemplazar a Oracle para pagos aunque escala horizontalmente?

MongoDB puede escalar horizontalmente, pero no garantiza consistencia ACID fuerte en transacciones financieras críticas. En sistemas de pagos se necesita asegurar que cada operación sea completamente consistente y no exista pérdida o duplicación de dinero.

---

### c) ¿Qué mecanismo técnico usa CockroachDB para mantener ACID en múltiples nodos distribuidos?

CockroachDB utiliza el algoritmo:

CockroachDB utiliza el algoritmo Raft Consensus, que permite coordinar múltiples nodos distribuidos asegurando replicación consistente y manteniendo propiedades ACID en todas las transacciones.
...
a) ¿Cuándo usarías MongoDB en Docker en lugar de MongoDB Atlas para el equipo de Yape?
 Usaría MongoDB en Docker para pruebas locales del equipo de desarrollo, cuando se quiere validar código sin depender de internet ni afectar la base real de Atlas.


b) ¿Qué ventaja tiene Atlas M0 sobre el contenedor Docker para el contexto universitario?
Atlas M0 es mejor para el contexto universitario porque es gratis, está en la nube y permite mostrar la base de datos desde cualquier computadora sin instalar MongoDB manualmente.
...
Si ejecuto docker stop, el contenedor se detiene pero los datos siguen existiendo. Si luego ejecuto docker rm, el contenedor se elimina y se pierden los datos si no se configuró un volumen. En Atlas, los datos permanecen en la nube aunque apague mi computadora.
...
**Enlace del video:** `https://https://youtu.be/algUXriVvvg?si=C2E6oMuHlhQ9rC3s`
