...
| **CÓDIGO DEL ESTUDIANTE:** |2221895643 | **NÚMERO DE CLASE:** | |
|---|---|---|---|
| **APELLIDOS Y NOMBRES:** |PAREDES HILARIO NOE | **FECHA ENTREGA:** |25/06/2026 |
...
Link de arquitectura draw.io y imagen:
https://drive.google.com/file/d/1w6EvaLRnnknNOrPNzxXSB4JJESmCJ42z/view?usp=sharing


| Componente del sistema | Tecnología elegida | Tipo BD/Herramienta | Por qué esta tecnología para Yape (2 líneas) |
|------------------------|-------------------|--------------------|--------------------------------------------|
| Core de pagos (3.2M transacciones/día, no puede perder dinero) |CockroachDB |NewSQL (Relacional Distribuida) |Porque necesitamos la seguridad de Oracle (transacciones ACID) pero que pueda crecer hacia los lados (escalabilidad horizontal) para aguantar los picos de 450 yapeos por segundo sin que se congelen los saldos. |
| Sesiones de login activo (15M usuarios, expira en 30 min) |Redis |NoSQL / En Memoria (Key-Value) |Como las sesiones duran solo 30 minutos, guardamos los tokens en la memoria RAM con Redis. Así, el login carga al instante (submilisegundos) y se borra solo cuando expira el tiempo. |
| Perfil del comerciante (bodega, restaurante, taxi — atributos distintos) |Mongo|NoSQL / Documental |Una bodega vende abarrotes, un taxi tiene placa y un restaurante tiene un menú. Como son datos tan distintos, MongoDB nos da la flexibilidad de meter todo en una misma colección sin dejar un montón de celdas vacías (NULL). |
| Historial de transacciones para análisis (18 TB/año) |Databricks (Delta Lake) |Lakehouse / Almacenamiento Analítico |Guardar 18 Terabytes al año en Oracle es carísimo y lento. Con Databricks lo metemos en un almacenamiento barato (Data Lake) y usamos Apache Spark para procesar todo ese historial en minutos con la arquitectura Medallion. |
| Red de detección de fraude (ciclo A→B→C→A en < 5 min) |Neo4j |NoSQL / Orientada a Grafos |El fraude con cuentas falsas se mueve rápido en círculos (de A a B, de B a C, y vuelve a A). Neo4j conecta los usuarios y transferencias como si fueran una red de amigos, detectando estas trampas al toque. |
| Dashboard ejecutivo (top 10 distritos, actualización diaria) |PostgreSQL |SQL Relacional (Data Mart) |Como este dashboard se actualiza solo una vez al día (en la madrugada), la cantidad de datos ya procesados es pequeña. No necesitamos un monstruo de Big Data aquí, con un Postgres clásico y barato basta y sobra. |
...
|------------|----------------|----------------------|----------------------------------------------------------------|
| Core de pagos (débito/crédito de saldos) |**CP** - (Consistencia y Tolerancia a Particiones)   |Disponibilidad |**Es correcto**. En finanzas, el dinero no puede fallar ni duplicarse. Si el sistema sufre una caída de red (partición), es mil veces mejor que la app de Yape te diga "Intente más tarde" (sacrificar disponibilidad) antes que dejarte hacer un pago con dinero que no tienes o que el saldo se duplique por error. |
| Historial "mis últimas 50 transacciones" |**AP**(Disponibilidad y Tolerancia a Particiones)|Consistencia |**Es correcto**. Aquí lo que importa es que la app cargue rápido y el usuario no se frustre. Si el usuario hace un yapeo y su historial tarda 5 o 10 segundos en mostrar la última línea (consistencia eventual), no pasa nada grave. El sacrificio es aceptable porque no afecta los saldos reales, solo a la visualización.  |
...
Respuesta.- Oracle escala de forma vertical (comprando un servidor más grande y caro), lo que genera un cuello de botella físico y bloqueos en las tablas de saldos con tantos usuarios. CockroachDB resuelve esto porque escala de forma horizontal (añadiendo servidores comunes en la nube que trabajan en equipo).

b) ¿Por qué MongoDB NO puede reemplazar a Oracle para el procesamiento de pagos aunque también escala horizontalmente?
Respuesta.- Porque MongoDB asegura la consistencia a nivel de un solo documento (un solo comercio o usuario). En un pago cruzado de Yape (donde restas saldo a uno y sumas a otro en milisegundos), MongoDB no puede garantizar de forma nativa el aislamiento ACID estricto entre múltiples servidores distribuidos, lo que podría duplicar o perder dinero en el camino.
...
Respuesta.- Usa el protocolo de consenso Raft (junto con Two-Phase Commit o 2PC)
...
Usaría MongoDB en Docker cuando necesito hacer pruebas locales sin afectar 
los datos reales en Atlas. Por ejemplo, si quiero probar queries destructivas 
o cargar datos de prueba, lo hago en Docker sin riesgo. También lo usaría 
cuando no tengo internet disponible, ya que Docker corre completamente en mi PC. 
En cambio, Atlas lo usaría para producción y trabajo en equipo donde todos 
necesitan acceder a la misma base de datos.


b) ¿Qué ventaja tiene Atlas M0 sobre el contenedor Docker para el contexto universitario?

Atlas M0 tiene la ventaja de ser accesible desde cualquier lugar con internet, 
sin necesidad de instalar nada en la PC. En el contexto universitario esto es 
ideal porque todos los integrantes del equipo pueden conectarse a la misma base 
de datos desde sus propias laptops sin configuraciones adicionales. Además, 
Atlas tiene una interfaz web (Browse Collections) que permite visualizar los 
documentos fácilmente, lo cual es útil para verificar el trabajo sin escribir 
código. Docker en cambio solo funciona en la máquina donde está instalado.
...
Si ejecuto docker stop yape-mongo-local, el contenedor se detiene pero los datos 
aún existen temporalmente. Sin embargo, si luego ejecuto docker rm yape-mongo-local, 
el contenedor se elimina permanentemente junto con todos sus datos, porque no se 
configuró un volumen externo con -v. Esto significa que todo lo insertado en Docker 
se pierde para siempre.

En cambio, los datos de Atlas no se ven afectados para nada, ya que están 
almacenados en la nube de AWS en São Paulo. Sin importar lo que pase en mi PC 
local, los documentos en Atlas siguen intactos y accesibles desde cualquier lugar.
