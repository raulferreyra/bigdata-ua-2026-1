...
**Nombre(s)**: Armando Jheferson Carbajal Campomanes  
...
_Respuesta_:  
Big Data es el conjunto de herramientas que sirven para guardar y analizar cantidades gigantescas de datos que llegan demasiado rápido y en formatos muy desordenados, tanto así que una computadora normal no podría soportarlo.

La diferencia principal es cómo se guardan y crecen los datos:
- En una base tradicional (como SQL Server), todo se guarda en un solo servidor central. Si te quedas sin espacio, tienes que comprar un servidor más caro y potente. Además, te obliga a usar tablas fijas (filas y columnas).
- En Big Data, la información se reparte entre varias computadoras normales conectadas a la vez. Si necesitas más espacio, solo conectas otra computadora barata a la red. También te permite guardar archivos en bruto (fotos, textos, audios) sin ordenarlos primero.
